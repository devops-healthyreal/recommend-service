                    ...
                    withCredentials([string(credentialsId: "${SONAR_TOKEN_CREDENTIAL_ID}", variable: 'SONAR_TOKEN_VAR')]) {
                        sh '''                    
                            ...
                            while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
                                QG_STATUS=$(curl -s -u ${SONAR_TOKEN_VAR}: "https://sonarcloud.io/api/qualitygates/project_status?projectKey=${PROJECT_KEY}" | grep -o '"status":"[^"]*"' | cut -d'"' -f4)
                            ...
                            echo "Quality Gate check timeout"
                            exit 1
                        '''
                    }
                }
